...
function showResult(parsed,filename){
  ...
  lastDecodedResult=decoded(parsed.best);document.getElementById('results').innerHTML=lastDecodedResult.map(resultHtml).join('');document.getElementById('kennelSaveCard').hidden=false;
...
// v11 My Kennel integration
const KENNEL_URL='https://mxboguiriifkmsmcusjt.supabase.co',KENNEL_KEY='sb_publishable_gZwUnRiKV2Ww2wqAXs9mBA_Z8lFw0LV';
const kennelClient=window.supabase?window.supabase.createClient(KENNEL_URL,KENNEL_KEY):null;
let kennelSession=null,lastDecodedResult=null;
if(kennelClient){
  kennelClient.auth.onAuthStateChange((_e,s)=>{kennelSession=s;renderKennelSaveAuth()});
  kennelClient.auth.getSession().then(x=>{kennelSession=x.data.session;renderKennelSaveAuth()});
}
function renderKennelSaveAuth(){
  const card=document.getElementById('kennelSaveCard');if(!card)return;
  document.getElementById('kennelLogin').hidden=!!kennelSession;
  document.getElementById('kennelSaveForm').hidden=!kennelSession;
  document.getElementById('kennelAuthText').textContent=kennelSession?'Signed in as '+kennelSession.user.email+'.':'Sign in to save this verified result privately.';
}
document.getElementById('kennelSignIn').onclick=async()=>{
  const m=document.getElementById('kennelAuthText');m.textContent='Signing in…';
  const {error}=await kennelClient.auth.signInWithPassword({email:document.getElementById('kennelEmail').value.trim(),password:document.getElementById('kennelPassword').value});
  if(error)m.textContent=error.message;
};
document.getElementById('saveEelogKennel').onclick=async()=>{
  const m=document.getElementById('kennelSaveMessage');
  if(!kennelSession||!lastDecodedResult)return m.textContent='Read an EE.log and sign in first.';
  const name=document.getElementById('saveKubrowName').value.trim();if(!name)return m.textContent='Enter the Kubrow name.';
  const by=slot=>lastDecodedResult.find(x=>x.slot===slot);
  const payload={
    owner_id:kennelSession.user.id,name,
    breed:document.getElementById('saveBreed').value.trim()||null,
    pattern:document.getElementById('savePattern').value.trim()||null,
    primary_colour:COLOURS[by('Primary').entry.key].name,
    secondary_colour:COLOURS[by('Secondary').entry.key].name,
    tertiary_colour:COLOURS[by('Tertiary').entry.key].name,
    eye_colour:COLOURS[by('Eyes').entry.key].name,
    accent_colour:COLOURS[by('Accent / custom').entry.key].name,
    primary_id:by('Primary').entry.key,secondary_id:by('Secondary').entry.key,
    tertiary_id:by('Tertiary').entry.key,eye_id:by('Eyes').entry.key,
    accent_id:by('Accent / custom').entry.key,verification_source:'eelog',
    trade_status:'private',is_public:false
  };
  m.textContent='Saving…';
  const {error}=await kennelClient.from('kennel_kubrows').insert(payload);
  m.textContent=error?error.message:'Saved to My Kennel.';
};
