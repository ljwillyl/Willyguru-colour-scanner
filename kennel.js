...
async function deleteRecord(id,name){
  ...
  if(error)return alert(error.message);await loadKennel();
}
...
function renderList(){
  ...
  $('kennelList').innerHTML=shown.map(r=>`
    <article class="kubrow">
      ...
      <div class="tags"><span class="tag ${['eelog','hybrid'].includes(r.verification_source)?'verified':''}">${esc(r.verification_source.toUpperCase())}</span><span class="tag">${esc(r.trade_status.replaceAll('_',' '))}</span></div></div>
      <div class="colourGrid">
        <div class="colour"><small>Primary</small>${esc(r.primary_colour||'Unknown')}</div>
        <div class="colour"><small>Secondary</small>${esc(r.secondary_colour||'Unknown')}</div>
        <div class="colour"><small>Tertiary</small>${esc(r.tertiary_colour||'Unknown')}</div>
      </div>
      <div class="meta">Eyes: ${esc(r.eye_colour||'Unknown')} · Accent: ${esc(r.accent_colour||'Unknown')} · Imprints: ${r.imprints_remaining??'Unknown'}</div>
      <div class="kubrowActions"><button class="secondary" data-edit="${r.id}">Edit</button><button class="danger" data-delete="${r.id}" data-name="${esc(r.name)}">Delete</button></div>
    </article>`).join('');
  ...
  $('verifiedCount').textContent=records.filter(r=>['eelog','hybrid'].includes(r.verification_source)).length;
